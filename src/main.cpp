...
/* SPACESHIPS	*/
    ...
    osg::ref_ptr<osg::Node> xWing (osgDB::readNodeFile("model/StarWars/star wars x-wing.3ds"));
    if(xWing == NULL) {
        cerr<<"Pas de xWing"<< endl;
        return 1;
    }
    osg::ref_ptr<osg::Node> tFighter (osgDB::readNodeFile("model/mechant/TF_3DS02.3ds"));
    if(tFighter == NULL) {
        cerr<<"Pas de tFighter"<<endl;
        return 1;
    }
